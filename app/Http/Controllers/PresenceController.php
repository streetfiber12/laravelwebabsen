...
{
    public function notAttendanceTime()
    {
        // 1. Dapatkan hari ini dalam format bahasa Indonesia
        $hari_ini = strtolower(\Carbon\Carbon::now()->locale('id')->dayName);

        // 2. Dapatkan semua jadwal absensi
        $allAttendances = Attendance::all();

        // 3. Filter jadwal absensi yang TIDAK berlaku untuk hari ini
        $notActiveAttendances = $allAttendances->filter(function ($attendance) use ($hari_ini) {
            return !in_array($hari_ini, $attendance->days);
        });

        // 4. Dapatkan daftar position_id dari jadwal yang tidak aktif
        $notActivePositionIds = $notActiveAttendances->pluck('positions')->flatten()->pluck('id')->unique();

        // 5. Dapatkan user (guru/pegawai) yang posisinya tidak aktif hari ini
        $users = \App\Models\User::query()
            ->whereHas('position', function ($query) use ($notActivePositionIds) {
                $query->whereIn('position_id', $notActivePositionIds);
            })
            ->get();

        return view('presences.not-attendance-time', [
            "title" => "Daftar Guru Bukan Waktu Absensi",
            "users" => $users,
            "hari_ini" => $hari_ini
        ]);
    }
    ...
    public function notPresent(Attendance $attendance)
    {
        ...
        $dayOfWeek = strtolower(\Carbon\Carbon::parse($byDate)->locale('id')->dayName);
        
        // PENTING: Periksa apakah hari ini adalah hari kerja untuk absensi ini
        if (!in_array($dayOfWeek, $attendance->days)) {
            // Jika bukan hari kerja, kembalikan array kosong
